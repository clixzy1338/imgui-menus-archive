...
bool ImGui::Keybind(const char* label, int* key)
{
    ...
    const ImRect rect(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(width, 26.f));
    ...
    ImRect clickable(ImVec2(rect.Max.x - 15 - label_size.x, rect.Min.y), rect.Max);
    ...
    window->DrawList->AddText(rect.Min + ImVec2(0, 3), GetColorU32(it_anim->second.text), label);

    PushStyleColor(ImGuiCol_Text, c::accent);

    RenderTextClipped(clickable.Min, clickable.Max - ImVec2(1, 0), buf_display, NULL, NULL, ImVec2(0.5f, 0.5f));
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    if (axis == ImGuiAxis_X)
        ...
        grab_rect = ImRect(bb.Min.x - 7, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x - 7, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(w, 26));
    ...
    it_anim->second.offset_circle = ImLerp(it_anim->second.offset_circle, *v ? 10.f : 30.f, g.IO.DeltaTime * 6.f);
    it_anim->second.circle = ImLerp(it_anim->second.circle, *v ? c::accent : c::text::text, g.IO.DeltaTime * 6.f);
    it_anim->second.alpha = ImLerp(it_anim->second.alpha, *v ? 0.0f : 1.f, g.IO.DeltaTime * 6.f);
    it_anim->second.check_size = ImLerp(it_anim->second.check_size, *v ? ImVec2(0.f, 0.f) : ImVec2(1.f, 1.f), g.IO.DeltaTime * 6.f);

    it_anim->second.text = ImLerp(it_anim->second.text, *v || s == label ? s == label ? c::accent : c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);
    ...
    const ImRect check_bb(pos, pos + ImVec2(w, 26));

    GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2(w - 40 + it_anim->second.check_size.x, 3 + it_anim->second.check_size.y), check_bb.Max - ImVec2(it_anim->second.check_size.x, 3 + it_anim->second.check_size.y), GetColorU32(c::accent, 0.2f), 30.f);
    GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2(w - 40, 3), check_bb.Max - ImVec2(0, 3), GetColorU32(c::checkbox::background, it_anim->second.alpha), 30.f);

    GetWindowDrawList()->AddCircleFilled(check_bb.Min + ImVec2(w - it_anim->second.offset_circle, 13), 6.f, GetColorU32(it_anim->second.circle), 30.f);

    PushClipRect(total_bb.Min, total_bb.Max, true);
    GetWindowDrawList()->AddText(ImVec2(check_bb.Min.x, check_bb.Min.y + (26 - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.text), label);
    PopClipRect();
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        {
            // Draw
            window->DrawList->AddRectFilled(bb.Min + ImVec2(GetStyle().WindowPadding.x, 0), bb.Max - ImVec2(GetStyle().WindowPadding.x, 0), GetColorU32(c::separator::line));
            if (g.LogEnabled)
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 26));
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, it_anim->second.opened_combo || s == label ? s == label ? c::accent : c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? (val * 33) + 17 : 0.f, g.IO.DeltaTime * 12.f);

    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - GetFrameHeight());

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(bb.Min + ImVec2(0, ((26 - CalcTextSize(preview_value).y)) / 2), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL, ImVec2(1.f, 0.0f));
    PopStyleColor();

    ImRotateStart();
    PushFont(font::icomoon[15]);
    GetWindowDrawList()->AddText(ImVec2(bb.Max.x - (10 + CalcTextSize("i").x) / 2, bb.Max.y - (26 + CalcTextSize("i").y) / 2), GetColorU32(it_anim->second.text), "i");
    PopFont();
    ImRotateEnd(1.57f * it_anim->second.arrow_roll);

    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Min + ImVec2(CalcTextSize(label).x + 10, 26), GetColorU32(c::child::background), 0.f);
    GetWindowDrawList()->AddRectFilledMultiColor(bb.Min + ImVec2(CalcTextSize(label).x, 0), bb.Min + ImVec2(CalcTextSize(label).x + 50, 26), GetColorU32(c::child::background), GetColorU32(c::child::background, 0.f), GetColorU32(c::child::background, 0.f), GetColorU32(c::child::background), 0.f);

    GetWindowDrawList()->AddText(ImVec2(bb.Max.x - w, bb.Min.y + (26 - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.text), label);
    ...
    ImGui::SetNextWindowPos(ImVec2(bb.Min.x, bb.Max.y + 14));
    ...
    PushStyleColor(ImGuiCol_WindowBg, c::tabs::background);
    PushStyleVar(ImGuiStyleVar_WindowRounding, c::tabs::rounding);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2((size_arg.y - CalcTextSize(hint).y), (size_arg.y - CalcTextSize(hint).y)) / 2;
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        it_anim->second.text = ImLerp(it_anim->second.text, s == label ? c::accent : c::text::text, g.IO.DeltaTime * 6.f);


        GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(0, 4), GetColorU32(it_anim->second.text), label);

        GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(GetContentRegionMax().x - (50 + CalcTextSize(buf).x), 4), GetColorU32(c::accent, 0.3f), buf);
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    GetForegroundDrawList()->AddCircleFilled(ImVec2(it_anim->second.hue_bar + bar_pos.x, bar_pos.y + 9), 6.5f, GetColorU32(c::text::text_active), 30.f);
    ...
    if (alpha_bar)
    {
        ...
        GetForegroundDrawList()->AddCircleFilled(ImVec2(it_anim->second.alpha_bar + bar_pos.x, bar1_bb.Min.y + 11.0f), 6.5f, GetColorU32(c::text::text_active), 100.f);
    }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImRect rect(pos, pos + ImVec2(width, 26));

    const ImRect clickable(rect.Min + ImVec2(width - 20, 3), rect.Max - ImVec2(0, 3));
    ...
    GetWindowDrawList()->AddRectFilled(clickable.Min, clickable.Max, GetColorU32(col_rgb), c::picker::rounding);

    RenderColorRectWithAlphaCheckerboard(window->DrawList, clickable.Min, clickable.Max, GetColorU32(col_rgb), 10, ImVec2(0.f, 0.f), c::picker::rounding);
